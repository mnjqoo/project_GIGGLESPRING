						...
						<tbody>
							<c:forEach var="reviewlist" items="${reviewList}">
								<tr id="productDetailReviewTitle">
									...
									<td class="detailReviewTh5">
									<c:choose>
										<c:when test="${reviewlist.review_star == 0 }">
											아직 구매 후기가 없어요!
										</c:when>
										<c:otherwise>
											<c:choose>
			                                    <c:when test="${reviewlist.review_star / 1 != 0}">
			                                       <c:forEach begin="1" end="${reviewlist.review_star / 1}">
			                                          <span class="fa fa-star checked"></span>
			                                       </c:forEach>
			                                    </c:when>
			                                 </c:choose>
			                                 <c:choose>
			                                 <c:when test="${reviewlist.review_star / 1 != 0}">
			                                    <c:forEach begin="1" end="${5-(reviewlist.review_star / 1)}">
			                                          <span class="fa fa-star-o" style="color: orange;"></span>
			                                       </c:forEach>
			                                    </c:when>   
			                                 </c:choose>
